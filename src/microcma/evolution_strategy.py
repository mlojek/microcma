...
from . import sampler, transformations
from .constraints_handler import (AugmentedLagrangian, BoundNone, BoundPenalty,
                                  BoundTransform)
...
class _CMASolutionDict_functional(_SolutionDict):
    ...
    def insert(
        self, key, geno=None, iteration=None, fitness=None, value=None, cma_norm=None
    ):
...
_CMASolutionDict = (
    _CMASolutionDict_functional if use_archives else _CMASolutionDict_empty
)
...
class CMAEvolutionStrategy(interfaces.OOOptimizer):
    ...
    def stop(
        self, check=True, ignore_list=(), check_in_same_iteration=False, get_value=None
    ):
        ...
        if (
            ...
            self.callbackstop = utils.ListOfCallables(
                self.opts["termination_callback"]
            )(self)

        self._stopdict._get_value = (
            get_value  # a hack to avoid passing arguments down to _add_stop and back
        )
        # check_on_same_iteration == False makes como code much faster
        res = self._stopdict(
            self,
            check_in_same_iteration
            or get_value
            or (  # update the stopdict and return a Dict (self)
    ...
    def __init__(self, x0, sigma0, inopts=None, options=None):
        ...
        opts = CMAOptions(
            inopts
        ).complement()  # CMAOptions() == fmin([],[]) == defaultOptions()
        ...
        assert (
            isinstance(opts["fixed_variables"], dict) or opts["fixed_variables"] is None
        )
        ...
        if not utils.is_nan(opts["seed"]):
            if self.opts["randn"] is np.random.randn:
                if (
                    not opts["seed"]
                    or opts["seed"] is time
                    or str(opts["seed"]).startswith("time")
                ):
                    np.random.seed()
        ...
        self.gp = transformations.GenoPheno(
            self.N_pheno,
        ...
        if not self.gp.isidentity:
            warnings.warn(
                "genotype-phenotype transformations induced by {0}\n may not"
                ...
                "\n`ScaleCoordinates` or `FixVariables` in `cma.fitness_transformations`.".format(
                    """
                        opts['scaling_of_variables'],
                        opts['typical_x'],
                        opts['fixed_variables'],
        ...
        self.mean_after_tell = np.array(
            self.mean, copy=True
        )  # to separate any after-iteration change
        ...
        self.mean_shift_samples = (
            True
            if (
                isinstance(self.adapt_sigma, CMAAdaptSigmaTPA)
                or opts["mean_shift_line_samples"]
            )
            else False
        )
        ...
        if len(opts["integer_variables"]):
            ...
            self.integer_centering = IntegerCentering(
                self
            )  # read opts['integer_variables'] and use boundary handler
        else:
        ...
        self.countevals = (
            ...
            if not isinstance(opts["verb_append"], bool)
            else 0
        )
        self.pc = np.zeros(N)
        self.pc2 = np.zeros(N)
        ...
        if 1 < 3:  # new version with class
            self.sigma_vec0 = eval_vector(
                self.opts["CMA_stds"], opts, N
            )  # may be a scalar
            ...
            if np.isfinite(self.opts["CMA_dampsvec_fac"]):
                self.sigma_vec *= np.ones(N)  # make sure to get a vector
            self.sigma_vec0 = (
                self.sigma_vec if np.isscalar(self.sigma_vec) else self.sigma_vec.copy()
            )
        if self.opts["CMA_diagonal"]:  # is True or > 0
            ...
            if self.opts["CMA_diagonal"] is True:
                self.sp.weights.finalize_negative_weights(
                    N, self.sp.c1_sep, self.sp.cmu_sep, pos_def=False
                )
            ...
            if 11 < 3:
                if (
                    hasattr(self.opts["vv"], "__getitem__")
                    and "sweep_ccov" in self.opts["vv"]
                ):
            ...
            if self.opts["CMA_sampler"] is None:
                self.sm = sampler.GaussFullSampler(
                    stds * np.ones(N),
                    lazy_update_gap=(
                        1.0 / (self.sp.c1 + self.sp.cmu + 1e-23) / self.N / 10
                ...
                p = self.sm.parameters(
                    mueff=self.sp.weights.mueff, lam=self.sp.weights.lambda_
                )
                ...
                try:
                    self.sm = self.opts["CMA_sampler"](
                        stds * np.ones(N), **self.opts["CMA_sampler_options"]
                    )
                except:
                    if max(stds) > min(stds):
                        utils.print_warning(
                            "different initial standard"
    ...
    for sample distribution"""
                    ...
                    self.sm = self.opts["CMA_sampler"](
                        N, **self.opts["CMA_sampler_options"]
                    )
            else:  # CMA_sampler is already initialized as class instance
                self.sm = self.opts["CMA_sampler"]
            if not isinstance(
                self.sm, interfaces.StatisticalModelSamplerWithZeroMeanBaseClass
            ):
                utils.print_warning(
    ...
    likely to lead to an exception later on. """
                    % (
                        str(
                            type(
                                interfaces.StatisticalModelSamplerWithZeroMeanBaseClass
                            )
                        ),
                        str(type(self.sm)),
                    )
        ...
        self._stds_into_limits(
            warn=global_verbosity > 0
        )  # put stds into [minstd, maxstd]
        ...
        if np.any(relative_stds > 1):
            ...
            s = (
                ...
                % (
                    ...
                    if len(idx) > 1
                    else (" sigma0 x stds is", " %s" % str(idx[0]))
                )
            )
            warnings.warn(s)
        ...
        self.logger = CMADataLogger(
            ...
            expensive_modulo=opts["verb_log_expensive"],
        ).register(self)
        ...
        if opts["verb_disp"] > 0 and opts["verbose"] >= 0:
            ...
            print(
                ...
                + "CMA-ES"
                + " (mu_w=%2.1f,w_1=%d%%)"
                % (self.sp.weights.mueff, int(100 * self.sp.weights[0]))
                + " in dimension %d (seed=%s, %s)"
                % (N, str(opts["seed"]), time.asctime())
            )  # + func.__name__
    ...
    def _set_x0(self, x0):
        ...
        self.x0 = array(x0, dtype=float, copy=True)
        ...
        if len(self.x0) <= 1:
            utils.print_warning(
                "Initial solution argument x0=%s.\n"
                "CAVEAT: Optimization in 1-D is poorly tested." % str(self.x0)
            )
        try:
        ...
        for i, s in enumerate(self.stds):
            ...
            if found:
                ...
                if s == 0 or not np.isfinite(self.sigma_vec.scaling[i] * sb):
                    warnings.warn(
                        "_stds_into_limits: sigmavec update omitted due to "
                        "an inadmissible value s={0}, scaling[i]={1}, sb={2}, stds={3}".format(
                            s, self.sigma_vec.scaling[i], sb, self.stds
                        )
                    )
                else:
                ...
                if warn:
                    warnings.warn(
                        "Sampling standard deviation i={0} at iteration {1}"
                        " change by {2} to stds[{3}]={4}".format(
                            i, self.countiter, sb / s, i, self.stds[i]
                        )
                    )
    ...
    def _set_C_from(self, es, scaling=True):
        ...
        if es.N != self.N:
            warnings.warn(
                "setting C with dimension {0} != {1} (the current "
                "dimension). This is likely to fail.".format(es.N, self.N)
            )
        if scaling:
            self.sigma_vec = transformations.DiagonalDecoding(es.sigma_vec.scaling)
            if np.any(self.pc2 != 0):
                warnings.warn(
                    "self.pc2={0} may not be compatible with changed sigma_vec".format(
                        self.pc2
                    )
                )
        try:
            ...
            if np.any(self.pc != 0):
                warnings.warn(
                    "self.pc={0} may not be compatible with changed C".format(self.pc)
                )
    ...
    def ask(self, number=None, xmean=None, sigma_fac=1, gradf=None, args=(), **kwargs):
        ...
        pop_pheno = [
            self.gp.pheno(x, copy=True, into_bounds=self.boundary_handler.repair)
            for x in pop_geno
        ]
            ...
            try:
                ...
                try:
                    if self.last_iteration_with_gradient == self.countiter:
                        utils.print_warning(
                            "gradient is used several times in " + "this iteration",
                            iteration=self.countiter,
                ...
                if not self.gp.isidentity or (
                    self.boundary_handler is not None
                    ...
                    elif self.boundary_handler is None or isinstance(
                ...
                if q:
                    ...
                    pop_geno[index_for_gradient] = xmean - self.sigma * (
                        self.N / q
                    ) ** 0.5 * (self.sigma_vec * np.dot(self.sm.B, self.sm.D * v))
                    if 11 < 3 and self.opts["vv"]:
                        # gradient direction
                        q = sum(
                            (
                                np.dot(self.sm.B.T, self.sigma_vec**-1 * grad_at_mean)
                                / self.sm.D
                            )
                            ** 2
                        )
                        pop_geno[index_for_gradient] = (
                            xmean - self.sigma * (self.N / q) ** 0.5 * grad_at_mean
                            if q
                            else xmean
                        )
                else:
                ...
                if 11 < 3:
                    ...
                    print(
                        "      (x-m-g)/||g||=",
                        (pop_pheno[index_for_gradient] - self.mean - grad_at_mean)
                        / sum(grad_at_mean**2) ** 0.5,
                    )
            except AttributeError:
                warnings.warn(
                    "Gradient injection failed presumably due\n"
    ...
    def ask_geno(self, number=None, xmean=None, sigma_fac=1):
        ...
        if self.number_of_solutions_asked == 0:
            self.number_of_injections = len(self.pop_injection_directions) + len(
                self.pop_injection_solutions
            )
        ...
        if self.opts["conditioncov_alleviate"]:
            self.alleviate_conditioning_in_coordinates(
                self.opts["conditioncov_alleviate"][0]
            )
        ...
        if number > 1 and (
            xmean_arg is None or Mh.vequals_approximately(xmean_arg, self.mean)
        ):
            if (
                self.countiter < 4
                and len(self.pop_injection_directions) > self.popsize - 2
            ):
                utils.print_warning(
            ...
            while self.pop_injection_directions:
                ...
                if (
                    self.mahalanobis_norm(y)
                    > self.N**0.5 * self.opts["CMA_injections_threshold_keep_len"]
                ):
            ...
            while self.pop_injection_solutions:
                arinj.append(
                    (self.pop_injection_solutions.pop(0) - self.mean) / self.sigma
                )
            if self.mean_shift_samples and (self.countiter > 1 or len(arinj) > 1):
                ...
                arinj[0] *= (
                    self._random_rescaling_factor_to_mahalanobis_size(arinj[0])
                    / self.sigma
                )
            ...
            assert (
                self.countiter < 2
                or not self.mean_shift_samples
        ...
        if (
            self.opts["verbose"] > 4
            and self.countiter < 3
            and len(arinj)
            and self.adapt_sigma is not CMAAdaptSigmaTPA
        ):
            utils.print_message(
                "   %d pre-injected solutions will be used (popsize=%d)"
                % (len(arinj), len(ary))
            )
    ...
    def random_rescale_to_mahalanobis(self, x):
        ...
        if any(x):  # let's not divide by zero
            x *= sum(
                self.opts["randn"](1, len(x))[0] ** 2
            ) ** 0.5 / self.mahalanobis_norm(x)
        x += self.mean
        return x
    ...
    def _random_rescaling_factor_to_mahalanobis_size(self, y):
        ...
        if not any(y):
            ...
            return 1.0
        return np.sum(
            self.opts["randn"](1, len(y))[0] ** 2
        ) ** 0.5 / self.mahalanobis_norm(y)
    ...
    def get_mirror(self, x, preserve_length=False):
        ...
        try:
            ...
            dx = (
                self.gp.geno(x, from_bounds=self.boundary_handler.inverse, copy=True)
                - self.mean
            )
    ...
    def ask_and_eval(
        ...
        if parallel_mode:
            ...
            if hasattr(func, "evaluations"):
                self.countevals += (
                    func.evaluations - evals0 - self.popsize
                )  # why not .sp.popsize ?
            ...
            while f is None or not is_feasible(x, f):  # rejection sampling
                ...
                if 11 < 3:
                    ...
                    length_normalizer = self.N**0.5 / self.mahalanobis_norm(
                        x - xmean
                    )  # self.const.chiN < N**0.5, the constant here is irrelevant (absorbed by kappa)
                ...
                f = (
                    func(x, *args)
                    if kappa == 1
                    else func(xmean + kappa * length_normalizer * (x - xmean), *args)
                )
                        ...
                        + [
                            (
                                ...
                                if kappa == 1
                                    ...
                                    xmean + kappa * length_normalizer * (x - xmean),
                                    *args
                                )
                            )
        ...
        if any(f is None or utils.is_nan(f) for f in fit):
            idxs = [
                i for i in range(len(fit)) if fit[i] is None or utils.is_nan(fit[i])
            ]
            utils.print_warning(
                "f-values %s contain None or NaN at indices %s"
                % (str(fit[:30]) + ("..." if len(fit) > 30 else ""), str(idxs)),
                "ask_and_tell",
                "CMAEvolutionStrategy",
                self.countiter,
            )
    ...
    def _prepare_injection_directions(self):
        ...
        if self.sp.lam_mirr and (
            self.opts["CMA_mirrormethod"] == 2
            or (
                self.opts["CMA_mirrormethod"] == 1
                and (  # replacement for direct selective mirrors
                    not hasattr(self, "_mirrormethod1_done")
                    or self._mirrormethod1_done < self.countiter - 1
                )
            )
        ):
            i0 = len(ary)
    ...
    def get_selective_mirrors(self, number=None):
        ...
        for i in range(1, number + 1):
            if (
                "all-selective-mirrors" in self.opts["vv"]
                or self.fit.idx[-i] not in self._indices_of_selective_mirrors
            ):
                res.append(self.mean_old - self.pop[self.fit.idx[-i]])
        assert len(res) >= number - len(self._indices_of_selective_mirrors)
        return res

    def limit_integer_relative_deltas(
        self, dX, threshold=None, recombination_weight_condition=None
    ):
    ...
    def tell(self, solutions, function_values, check_points=None, copy=False):
        ...
        if lam != len(function_values):
            raise ValueError(
                "#f-values = %d must equal #solutions = %d"
        ...
        if not np.isscalar(function_values[0]):
            try:
                ...
                utils.print_message(
                    "function values=%s" % function_values,
                    method_name="tell",
                    class_name="CMAEvolutionStrategy",
                    verbose=9,
                    iteration=self.countiter,
                )
                raise
        if any(f is None or utils.is_nan(f) for f in function_values):
            ...
            idx_nan = [
                ...
                if f is not None and utils.is_nan(f)
            ]
            m = np.median(
                [f for f in function_values if f is not None and not utils.is_nan(f)]
            )
            utils.print_warning(
                "function values with index %s/%s are nan/None and will be set to the median value %s"
                % (str(idx_nan), str(idx_none), str(m)),
                "ask",
                "CMAEvolutionStrategy",
                self.countiter,
            )
        ...
        if not all(np.isfinite(float(val)) for val in function_values):
            idx = [
                i for i, f in enumerate(function_values) if not np.isfinite(float(f))
            ]
            utils.print_warning(
                "function values with index %s are not finite but %s."
                % (str(idx), str([function_values[i] for i in idx])),
                "ask",
                "CMAEvolutionStrategy",
                self.countiter,
            )
        ...
        if 1 < 3 and (
            lam > sp.popsize + 2
            or lam < sp.popsize - 2
            or (lam < sp.popsize and lam < 5)
        ):  # see above
            m = "The number of solutions passed to `tell` should"
            utils.print_warning(
                "{0} generally be the same as (or close to) the population size,"
        ...
        if (
            lam < sp.weights.mu
        ):  # rather decrease cmean instead of having mu > lambda//2
            raise ValueError(
                "not enough solutions passed to function tell"
                " (passed solutions={0} < mu={1})".format(lam, sp.weights.mu)
            )

        self.countiter += 1  # >= 1 now
        self.countevals += lam * self.evaluations_per_f_value
        self.best.update(
            solutions,  # caveat: these solutions may be out-of-bounds
            self.sent_solutions,
            function_values,
            self.countevals,
        )
        ...
        fit.fit = sorted(
            function_values
        )  # was: array(function_values)[fit.idx] which can falsely trigger tolflatfitness
        ...
        fit.hist.insert(
            0, fit.fit[0]
        )  # FIXED caveat: this may neither be the best nor the best in-bound fitness
        fit.median = (
            fit.fit[len(fit.fit) // 2]
            if len(fit.fit) % 2
        ...
        for k, s in enumerate(solutions):  # use phenotype before Solution.repair()
            if 1 < 3:
                ...
                try:
                    self.archive.insert(
                        s, value=self.sent_solutions.pop(s), fitness=function_values[k]
                    )
        ...
        if self.opts["CMA_elitist"] == "initial":
            if not hasattr(self, "f0"):
                ...
                utils.print_message(
                    "initial solution injected %f<%f" % (self.f0, fit.fit[0]),
                    "tell",
                    "CMAEvolutionStrategy",
                    self.countiter,
                    verbose=self.opts["verbose"],
        ...
        ):  # CAVEAT: check_points might be an index-list
            cmean = self.sp.cmean / min(
                1,
                ((self.opts["vv"] * N) ** 0.5 + 2)
                / (  # abuse of cmean
        ...
        if 11 < 3:  # plot length of mean - mold
            self.more_to_write += [
                self.sp.weights.mueff**0.5
                * sum(((1.0 / self.D) * np.dot(self.B.T, self.mean - mold)) ** 2) ** 0.5
                / self.sigma
                / N**0.5
                / cmean
            ]
        ...
        c1 = (
            self.opts["CMA_on"]
            * self.opts["CMA_rankone"]
            * self.sm.parameters(mueff=sp.weights.mueff, lam=sp.weights.lambda_).get(
                "c1", sp.c1
            )
        )  # mueff and lambda_ should not be necessary here
        cmu = (
            self.opts["CMA_on"]
            * self.opts["CMA_rankmu"]
            * self.sm.parameters().get("cmu", sp.cmu)
        )
        if flg_diagonal:
        ...
        self.pc = (1 - cc) * self.pc + hsig * (
            (cc * (2 - cc) * self.sp.weights.mueff) ** 0.5 / self.sigma / cmean
        ) * (self.mean - mold) / self.sigma_vec.scaling
        dd_params = self.sigma_vec.parameters(
        ...
        self.pc2 = (1 - cc2) * self.pc2 + hsig * (
            (cc2 * (2 - cc2) * self.sp.weights.mueff) ** 0.5 / self.sigma / cmean
        ) * (self.mean - mold)
        ...
        if c1a + cmu > 0:
            ...
            if "inc_cmu_pos" in self.opts["vv"]:
                ...
                sampler_weights[sampler_weights > 0] *= (
                    1 + self.opts["vv"]["inc_cmu_pos"]
                )
                ...
                try:
                    ...
                    if i > 1 or not isinstance(self.adapt_sigma, CMAAdaptSigmaTPA):
                        ...
                        if sampler_weights_dd[i + 1] < 0:
                            sampler_weights_dd[i + 1] *= self.opts[
                                "CMA_active_injected"
                            ]
            for k, s in list(self._injected_solutions_archive.items()):
                if s["iteration"] < self.countiter - 2:
                    ...
                    if (
                        not isinstance(self.adapt_sigma, CMAAdaptSigmaTPA)
            ...
            if flg_diagonal:
                self.sigma_vec.update(
                    ...
                    + list(
                        self.sm.transform_inverse(
                            pop_zero / (self.sigma * self.sigma_vec.scaling)
                        )
                    ),
                    np.log(2) * np.asarray(sampler_weights),
                    integer_indices=self.opts["integer_variables"],
                )  # log(2) is here for historical reasons
            else:
                pop_zero_encoded = pop_zero / (self.sigma * self.sigma_vec.scaling)
                if self.opts["CMA_diagonal_decoding"] and hasattr(
                    self.sm, "beta_diagonal_acceleration"
                ):
                    ws = [
                        self.opts["CMA_on"]
                        * self.opts["CMA_diagonal_decoding"]
                        / self.sm.beta_diagonal_acceleration
                        * w
                        for w in sampler_weights_dd
                    ]
                    self.sigma_vec.update(
                ...
                if CMAOptions._ps_for_pc:  # experimental
                    try:
                        ...
                        pc = fac * self.sm.transform(
                            self.adapt_sigma.ps
                        )  # ps is not yet updated
                    except:
                        raise
                self.sm.update(
                    [(c1 / (c1a + 1e-23)) ** 0.5 * pc]  # c1a * pc**2 gets c1 * pc**2
                    + list(pop_zero_encoded),
                    sampler_weights,
                )
            if any(np.asarray(self.sm.variances) < 0):
                raise RuntimeError(
                    "A sampler variance has become negative "
        ...
        try:
            self.sigma *= self.adapt_sigma.update2(
        ...
        if self.opts["stall_sigma_change_on_divergence_iterations"] and (
            self.fit.median_got_worse
            >= self.opts["stall_sigma_change_on_divergence_iterations"]
        ):
            # for the record only
        ...
        if 11 < 3:  # old min/maxstd code
            if any(
                self.sigma * self.sigma_vec.scaling * self.dC**0.5
                < np.asarray(self.opts["minstd"])
            ):
                self.sigma = max(
                    np.asarray(self.opts["minstd"]) / (self.sigma_vec * self.dC**0.5)
                )
                assert all(
                    self.sigma * self.sigma_vec * self.dC**0.5
                ...
                self.sigma = min(
                    np.asarray(self.opts["maxstd"]) / (self.sigma_vec * self.dC**0.5)
                )
        # g = self.countiter
        # N = self.N
        ...
        if (
            self.sigma * min(self.D) < self.opts["mindx"]
        ...
        if CMAOptions._stationary_sphere:
            if callable(CMAOptions._stationary_sphere):
                self.mean *= CMAOptions._stationary_sphere(
                    self.mean_old
                ) / CMAOptions._stationary_sphere(self.mean)
            else:
                self.mean *= np.sqrt(
                    sum(np.square(self.mean_old)) / sum(np.square(self.mean))
                )
        ...
        if (
            11 < 3
            and isinstance(self.boundary_handler, BoundTransform)
        ...
        if _new_injections:
            ...
            if (
                self.opts["verbose"] > 4
                and self.countiter < 3
                and not isinstance(self.adapt_sigma, CMAAdaptSigmaTPA)
                and len(self.pop_injection_directions)
            ):
                utils.print_message(
                    "   %d directions prepared for injection %s"
                    % (
                        len(self.pop_injection_directions),
                        (
        ...
        self._stoptolxstagnation.set_params(self.opts["tolxstagnation"]).update(
            self.mean
        )
        self._flgtelldone = True
        try:  # shouldn't fail, but let's be nice to code abuse
            ...
            warnings.warn(
                'CMAEvolutionStrategy.tell(countiter=%d): "timer" attribute '
                "not found, probably because `ask` was never called. \n"
                "Timing is likely to work only until `tell` is called (again), "
                "because `tic` will never be called again afterwards." % self.countiter
            )
            self.timer = utils.ElapsedWCTime()
    ...
    def inject(self, solutions, force=None):
        ...
        for solution in solutions:
            ...
            if len(solution) != self.N:
                raise ValueError(
                    "method `inject` needs a list or array"
    ...
    def result_pretty(self, number_of_runs=0, time_str=None, fbestever=None):
        ...
        s = (
            (" after %i restart" + ("s" if number_of_runs > 1 else "")) % number_of_runs
            if number_of_runs
            else ""
        )
        for k, v in self.stop().items():
            print(
                "termination on %s=%s%s"
                % (k, str(v), s + (" (%s)" % time_str if time_str else ""))
            )

        print(
            "final/bestever f-value = %e %e after %d/%d evaluations"
    ...
    def manage_plateaus(self, sigma_fac=1.5, sample_fraction=0.5):
        ...
        if not self._flgtelldone:
            utils.print_warning(
                "Inbetween `ask` and `tell` plateaus cannot"
    ...
    def alleviate_conditioning_in_coordinates(self, condition=1e8):
        ...
        if hasattr(self, "sm") and isinstance(self.sm, sampler.GaussFullSampler):
            ...
            utils.print_message(
                ...
                % (
                    old_coordinate_condition,
                    np.max(self.dC) / np.min(self.dC),
                    old_condition,
                    self.sm.condition_number,
                ),
                iteration=self.countiter,
            )
    ...
    def alleviate_conditioning(self, condition=1e12):
        ...
        try:
            ...
            self.pc2 = np.dot(
                tf_inv, self.pc2 / self.sigma_vec
            )  # below, sigma_vec goes into gp
            self._updateBDfromSM(self.sm)
        except NotImplementedError:
        ...
        self.gp._tf_matrix = (
            self.sigma_vec * tf.T
        ).T  # sig*tf.T .*-multiplies each column of tf with sig
        ...
        self.gp.tf_geno = (
            self._tfg
        )  # lambda x: np.dot(self.gp._tf_matrix_inv, x)  # not really necessary
    ...
    def isotropic_mean_shift(self):
        ...
        *systematic* selection (e.g. on a linear function) tends to
        ...
        if self._isotropic_mean_shift_iteration != self.countiter:
            ...
            self._isotropic_mean_shift *= (
                self.sp.weights.mueff**0.5 / self.sigma / self.sp.cmean
            )
        ...
        if modulo:
            ...
            if self.countiter > 0 and (
                ...
                if self.opts["verb_time"]:
                    ...
                    stime = (
                        str(int(toc // 60)) + ":" + ("%2.1f" % (toc % 60)).rjust(4, "0")
                    )
                else:
                    stime = ""
                print(
                    " ".join(
                        (
                            ...
                            "%4.1e"
                            % (
                                self.D.max() / self.D.min()
                                if not self.opts["CMA_diagonal"]
                                or self.countiter > self.opts["CMA_diagonal"]
    ...
    def plot(self, *args, **kwargs):
        ...
        try:
            ...
            utils.print_warning(
                "plotting failed with: {0}".format(sys.exc_info()),
                "plot",
                "CMAEvolutionStrategy",
            )
        return self
...
class _StopTolXStagnation(object):
    ...
    def __init__(self, x=None):
        ...
        self.delta = vals[0]
        self.time_delta_offset = vals[
            1
        ]  # should depend on dimension because of adaptation delays?
    ...
    def set_params(
        self,
        param_values,
        names=("delta", "time_delta_offset", "time_delta_frac", "time_delta_expo"),
    ):
    ...
    def update(self, x):
        ...
        delta = (
            self.delta
            * max((1, (self.count - self.count_x) / self.time_threshold)) ** 0.5
        )
...
class _CMAStopDict(dict):
    ...
    def _update(self, es):
        ...
        if (
            opts["verbose"] >= -9
            and opts["signals_filename"]
            and os.path.isfile(self.opts["signals_filename"])
        ):
            ...
            try:
                ...
                warnings.warn(
                    "SyntaxError when parsing the following expression with `ast.literal_eval`:"
                    "\n\n%s\n(contents of file %s)"
                    % (s, str(self.opts["signals_filename"]))
                )
            else:
                for key in list(d):
        ...
        self._addstop(
            "tolfacupx",
            any(
                sigma_x_sigma_vec_x_sqrtdC
                > es.sigma0 * es.sigma_vec0 * opts["tolfacupx"]
            ),
        )
        self._addstop(
            "tolx",
            all(sigma_x_sigma_vec_x_sqrtdC < opts["tolx"])
            and all(es.sigma * (es.sigma_vec.scaling * es.pc) < opts["tolx"]),
        ...
        self._addstop(
            "tolfun",
            current_fitness_range
            < opts["tolfun"]  # fit.fit is sorted including bound penalties
            and historic_fitness_range < opts["tolfun"],
        )
        self._addstop(
            "tolfunrel",
            current_fitness_range
            < opts["tolfunrel"] * (es.fit.median0 - es.fit.median_min),
            current_fitness_range if self._get_value else None,
        )
        self._addstop(
            "tolfunhist",
            len(es.fit.hist) > 9 and historic_fitness_range < opts["tolfunhist"],
        )
        if opts["tolstagnation"]:
            ...
            if l <= es.countiter:
                ...
                self._addstop(
                    ...
                    and (es.countevals - es.best.evals) / es.popsize
                    > es.opts["tolstagnation"]
                    / 2  # recorded best is in the first half of stagnation period
                    and len(es.fit.histbest) > 100
                    and 2 * l < len(es.fit.histbest)
                    and np.median(es.fit.histmedian[:l])
                    >= np.median(es.fit.histmedian[l : 2 * l])
                    and np.median(es.fit.histbest[:l])
                    >= np.median(es.fit.histbest[l : 2 * l]),
                )
        ...
        self._addstop(
            "tolupsigma",
            opts["tolupsigma"] and s > es.sigma0 * opts["tolupsigma"],
        ...
        if 1 < 3:
            ...
            #                )
            if (
                opts["CMA_diagonal"] is not True
                and es.countiter > opts["CMA_diagonal"]
                and (es.countiter % 1) == 0
            ):  # save another factor of two?
                i = es.countiter % N
                try:
                    self._addstop(
                        "noeffectaxis",
            ...
            self._addstop(
                "tolconditioncov",
                opts["tolconditioncov"]
                and es.D[-1] > opts["tolconditioncov"] ** 0.5 * es.D[0],
                opts["tolconditioncov"],
            )

            self._addstop(
                "callback", any(es.callbackstop), es.callbackstop
            ...
            if 1 < 3:
                if es.fit.fit[0] < es.fit.fit[int(0.75 * len(es.fit.fit))]:
                    ...
                    if (
                        ...
                        if (
                            11 < 3
                            and max(es.fit.fit) == min(es.fit.fit) == es.best.last.f
                        ):  # keep warning for historical reasons for the time being
                            utils.print_warning(
                                "flat fitness (f=%f, sigma=%.2e). "
                                "For small sigma, this could indicate numerical convergence. \n"
                                "Otherwise, please (re)consider how to compute the fitness more elaborately."
                                % (es.fit.fit[0], es.sigma),
                                iteration=es.countiter,
                            )
            if (
                11 < 3
            ):  # add stop condition, in case, replaced by above, subject to removal
                self._addstop(
                    "flat fitness",  # message via stopdict
                    len(es.fit.hist) > 9
                    and max(es.fit.hist) == min(es.fit.hist)
                    and max(es.fit.fit) == min(es.fit.fit),
                    "please (re)consider how to compute the fitness more elaborately if sigma=%.2e is large"
                    % es.sigma,
                )
...
def fmin_lq_surr2(
    ...
    for irun in range(1 + restarts["maxrestarts"]):
        ...
        if (
            es.countevals >= restarts["maxfevals"]
            or "ftarget" in es.stop(check=False)
...
all_stoppings = (
    []
)  # accessable via cma.evolution_strategy.all_stoppings, bound to change
    ...
    if 1 < 3:  # try: # pass on KeyboardInterrupt
        ...
        while True:  # restart loop
            ...
            for k in fmin_opts:
                ...
                # in 3.x, therefore
                fmin_opts.eval(
                    k, loc={"N": es.N, "popsize": opts["popsize"]}, correct_key=False
                )
            ...
            try:
                logger.persistent_communication_dict.update(
                    {"variable_annotations": objective_function.variable_annotations}
                )
            except AttributeError:
                pass
            ...
            if 11 < 3:
                if (
                    es.countiter == 0
                    and es.opts["verb_log"] > 0
                    and not es.opts["verb_append"]
                ):
            ...
            if 1 < 3:
                while not es.stop():  # iteration loop
                    ...
                    X, fit = es.ask_and_eval(
                        parallel_objective or objective_function,
                        args,
                        gradf=gradf,
                        evaluations=noisehandler.evaluations,
                        aggregation=np.median,
                        parallel_mode=parallel_objective,
                    )  # treats NaN with resampling if not parallel_mode
                    ...
                    if (
                        es.opts["verbose"] > 4
                    ):  # may be undesirable with dynamic fitness (e.g. Augmented Lagrangian)
                        if es.countiter < 2 or min(fit) <= es.best.last.f:
                            degrading_iterations_count = (
                                0  # comes first to avoid code check complaint
                            )
                        else:  # min(fit) > es.best.last.f:
                            degrading_iterations_count += 1
                            if degrading_iterations_count > 4:
                                utils.print_message(
                                    "%d f-degrading iterations (set verbose<=4 to suppress)"
                                    % degrading_iterations_count,
                                    iteration=es.countiter,
                                )
                    ...
                    if (
                        noise_handling
                    ):  # it would be better to also use these f-evaluations in tell
                        es.sigma *= (
                            noisehandler(X, fit, objective_function, es.ask, args=args)
                            ** fmin_opts["noise_change_sigma_exponent"]
                        )
                        ...
                        if 1 < 3:
                            ...
                            es.sp.cmean *= np.exp(
                                -noise_kappa_exponent * np.tanh(noisehandler.noiseS)
                            )
                            es.sp.cmean[es.sp.cmean > 1] = (
                                1.0  # also works with "scalar arrays" like np.array(1.2)
                            )
                    for f in callback:
                        f is None or f(es)
                    es.disp()
            ...
            if (
                irun - runs_with_small > fmin_opts["restarts"]["maxrestarts"]
            ...
            opts["popsize"] = (
                fmin_opts["incpopsize"] * es.sp.popsize
            )  # TODO: use rather options?
            try:
                opts["seed"] += 1
            except TypeError:
                pass
...
def fmin_con(
    ...
    **kwargs
    ...
    if "parallel_objective" in kwargs:
        raise ValueError(
            "`parallel_objective` parameter is not supported by cma.fmin_con"
        )
    if (
        post_optimization
        and h != no_constraints
        and (not isinstance(post_optimization, float) or post_optimization <= 0)
    ):
        raise ValueError(
            "When equality constraints are given, the argument"
    ...
    if archiving:
        archives = [
            _constraints_handler.ConstrainedSolutionsArchive(
                _constraints_handler._g_pos_max
            ),
            _constraints_handler.ConstrainedSolutionsArchive(
                _constraints_handler._g_pos_sum
            ),
            _constraints_handler.ConstrainedSolutionsArchive(
                _constraints_handler._g_pos_squared_sum
            ),
        ]
    else:
        archives = []
    ...
    def constraints(x):
        ...
        if _al.lam is None:  # TODO: better abide by an "official" interface?
            ...
            _al._equality = np.asarray(
                len(gvals) * [False] + len(hvals) * [True], dtype="bool"
            )
    ...
    def auglag(x):
        ...
        if all([gi <= 0 for gi in gvals]):
            best_feasible_solution.update(
                fval, x, info={"x": x, "f": fval, "g": gvals, "g_al": alvals}
            )
    ...
    if post_optimization:
        ...
        if all([gi <= 0 for gi in g_x_post]) and all(
            [hi**2 <= post_optimization**2 for hi in h_x_post]
        ):
            f_x_post = objective_function(x_post)
            es.best_feasible.update(
                f_x_post,
                x=x_post,
                info={"x": x_post, "f": f_x_post, "g": list(g_x_post) + list(h_x_post)},
            )
            return x_post, es
        else:
            utils.print_warning(
                "Post optimization was unsuccessful", verbose=es.opts["verbose"]
            )
    ...
    return (
        es.result.xfavorite,
        es,
    )  # do not return es.best_feasible.x because it could be quite bad
...
def fmin_con2(
    ...
    **kwargs_fmin
    ...
    if isinstance(find_feasible_first, dict) or isinstance(find_feasible_final, dict):
        raise ValueError(
            "Found an unexected `dict` as argument. Recheck the calling signature."
    ...
    logging = _al_set_logging(
        None, kwargs_fmin, kwargs_confit
