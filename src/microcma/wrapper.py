...
try:
    ...
    warnings.warn(
        'install `skopt` ("pip install scikit-optimize") ' "to use `SkoptCMAoptimizer`"
    )
else:
    ...
    def SkoptCMAoptimizer(
        ...
        if normalize:
            dimensions = list(
                map(lambda x: skopt.space.check_dimension(x, "normalize"), dimensions)
            )
        ...
        with cma.optimization_tools.EvalParallel2(
            func, number_of_processes=switch.get(n_jobs, n_jobs)
        ) as parallel_func:
