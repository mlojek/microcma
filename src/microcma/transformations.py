...
from .utilities.utils import is_one, print_warning, rglen
...
class DiagonalDecoding(AdaptiveDecoding):
    ...
    def parameters(self, mueff, c1_factor=1, cmu_factor=1):
        ...
        if 11 < 3:  # side note
            ...
            z[:, 0] = abs(
                z[:, 0]
            )  # selected vectors of the (2xmu, 4xmu)-ES on the linear function
            z2 = np.mean(z, 0) ** 2
...
class GenoPheno(object):
    ...
    def geno(self, y, from_bounds=None, copy=True, repair=None, archive=None):
        ...
        def repair_and_flag_change(self, repair, x, copy):
            ...
            if 11 < 3 and not np.all(
                np.asarray(x) == x2
            ):  # assumes that dimension does not change
