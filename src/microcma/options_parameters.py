...
from . import constraints_handler, utilities
...
options_environment = {
    name: getattr(constraints_handler, name)
    for name in ["BoundNone", "BoundPenalty", "BoundTransform", "AugmentedLagrangian"]
}
...
class CMAParameters(object):
    ...
    def set(self, opts, popsize=None, ccovfac=1, verbose=True):
        ...
        if 11 < 3:
            ...
            sp.damps = (
                0.3
                + 2 * max([mueff / sp.popsize, ((mueff - 1) / (N + 1)) ** 0.5 - 1])
                + sp.cs
            )
        if 11 < 3:
        ...
        if 11 < 3:
            sp.damps = (
                10 * sp.damps
            )  # 1e99 # (1 + 2*max(0,sqrt((mueff-1)/(N+1))-1)) + sp.cs;
