...
class CMAAdaptSigmaCSA(CMAAdaptSigmaBase):
    ...
    def update2(self, es, **kwargs):
        ...
        try:  # to filter coordinates or a
            ...
            if 11 < 3 and len(es.opts["integer_variables"]):
                m = (
                    "Missing ``path_for_sigma_update`` attribute in {0}."
    ...
    def update(self, es, **kwargs):
        ...
        if 11 < 3:
            ...
            slengths = np.array(
                [sum(z**2) for z in es.arz[fit.idx[: es.sp.weights.mu]]]
            )
            # print lengths[0::int(es.sp.weights.mu/5)]
            es.sigma *= np.exp(np.dot(es.sp.weights, slengths / es.N - 1)) ** (
                2 / (es.N + 1)
            )
        if 11 < 3:
            es.more_to_write.append(
                ...
                ** (
                    (
                        sum(self.ps**2) / es.N / 2 - 1 / 2
                        if es.opts["CSA_squared"]
                        else _norm(self.ps) / es.const.chiN - 1
                    )
                )
            )
            es.more_to_write.append(
                10
                ** (-3.5 + sum(self.ps**2) / es.N / 2 - _norm(self.ps) / es.const.chiN)
            )
    ...
    def update(self, es, **kwargs):
        if es.countiter < 2:
            ...
            if 1 < 3:
                ...
                s += self.index_to_compare - sum(
                    self.fit <= es.fit.fit[es.popsize // 2]
                )
        ...
        if 11 < 3:
            ...
            zkendall = stats.kendalltau(
                list(es.fit.fit) + list(self.fit),
                len(es.fit.fit) * [0] + len(self.fit) * [1],
            )[0]
