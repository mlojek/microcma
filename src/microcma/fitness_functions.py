...
class FitnessFunctions(
    object
):  # TODO: this class is not necessary anymore? But some effort is needed to change it
    ...
    def noisysphere(self, x, noise=2.10e-9, cond=1.0, noise_offset=0.10):
        ...
        return (
            self.elli(x, cond=cond) * np.exp(0 + noise * np.random.randn() / len(x))
    ...
    def cigar(self, x, rot=0, cond=1e6, noise=0):
        ...
        f = [
            (x[0] ** 2 + cond * sum(x[1:] ** 2))
            * np.exp(noise * np.random.randn(1)[0] / len(x))
            for x in x
        ]
    ...
    def elli(
        ...
        if not isscalar(x[0]):  # parallel evaluation
            return [
                self.elli(xx, rot, xoffset, cond, actuator_noise, beta_noise, both)
                for xx in x
            ]  # could save 20% overall
        x = np.asarray(x)
        N = len(x)
        if rot:
            x = rotate(x)
        ...
        ftrue = (
            sum(cond ** (np.arange(N) / (N - 1.0)) * (x + xoffset) ** 2)
            if N > 1
            else (x[0] + xoffset) ** 2
        )
        ...
        alpha = 0.49 + 1.0 / N
        felli = (
            np.random.rand() ** beta_noise
            * ftrue
            * max(1, (10.0**9 / (ftrue + 1e-99)) ** (alpha * np.random.rand()))
        )
        if both:
            return (felli, ftrue)
        return felli
    ...
    def grad_elli(self, x, *args):
        cond = 1e6
        N = len(x)
        return 2 * cond ** (np.arange(N) / (N - 1.0)) * np.asarray(x)
    ...
    def rosen0(self, x, alpha=1e2):
        ...
        x = np.asarray(x) + 1
        return sum(alpha * (x[:-1] ** 2 - x[1:]) ** 2 + (1.0 - x[:-1]) ** 2)
    ...
    def griewank(self, x):
        ...
        x = (600.0 / 5) * x
        return (
            1 - np.prod(np.cos(x / np.sqrt(1.0 + np.arange(len(x))))) + sum(x**2) / 4e3
        )
    ...
    def styblinski_tang(self, x):
        ...
        return (39.1661657037714171054273576010019 * len(x)) ** 1 + sum(
            x**4 - 16 * x**2 + 5 * x
        ) / 2
    ...
    def xinsheyang2(self, x, termination_friendly=True):
        ...
        """
        x = np.asarray(x)
        val = np.sum(np.abs(x)) * np.exp(
            -np.sum(np.sin(np.square(x)))
        )  # np.mean under the exponential makes the function much easier
    ...
    def binval(x, foffset=binary_foffset, optimum=binary_optimum_interval):
        ...
        """
        s = sum_(
            0 if optimum[0] <= val <= optimum[1] else 2**i for i, val in enumerate(x)
        )
...
class _F_0(object):
    ...
    def __init__(self, funID, dimension=10, suite="bbob", instance=1):
        ...
        self._input_parameters = dict(
            l for l in list(locals().items()) if l[0] != "self"
        )  # for the record
        import cocoex

        if suite not in cocoex.known_suite_names:
            raise ValueError(
                "Sorry, suite '{0}' is not known, choices are {1}".format(
                    suite, cocoex.known_suite_names
                )
            )
        self.suite = cocoex.Suite(
            suite,  # caveat: unknown suite may crash Python
