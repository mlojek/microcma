    ...
    if 11 < 3:  # newint does produce an error on some installations
        try:
            from builtins import (  # not list and object, by default builtins don't exist in Python 2
                ascii, bytes, chr, dict, filter, hex, input, int, map, next,
                oct, open, pow, range, round, str, super, zip)

            from future.builtins import (ascii, bytes, chr, dict, filter, hex,
                                         input, int, list, map, next, object,
                                         oct, open, pow, range, round, str,
                                         super, zip)
