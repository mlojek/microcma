        ...
        if callable_ is not None:
            ...
            if not self.target_hit_at and any(np.asarray(F) <= self.ftarget):
                self.target_hit_at = (
                    ...
                    - len(X)
                    + 1
                    + list(np.asarray(F) <= self.ftarget).index(True)
                )
...
class ComposedFunction(Function, list):
    ...
    def __call__(self, x, *args, **kwargs):
        Function.__call__(
            self, x, *args, **kwargs
        )  # for the possible side effects only
...
class StackFunction(Function):
    ...
    def _eval(self, x, *args, **kwargs):
        return self.f1(x[: self.n1], *args, **kwargs) + self.f2(
            x[self.n1 :], *args, **kwargs
        )
    ...
    def __init__(
        ...
        if multipliers is not None:
            if upper is not None:
                raise ValueError(
                    "Either `multipliers` or `upper` argument must be None"
                )
            if from_lower_upper not in (None, (0, 1)):
                warnings.warn(
                    "from_lower_upper={0} ignored because multipliers={1} were given".format(
                        from_lower_upper, multipliers
                    )
                )
            ...
            if upper is None and lower is None:
                raise ValueError(
                    "Either `multipliers` or `zero` or `upper` or `lower`"
            ...
            if len(idx):
                raise ValueError(
                    "`upper` value(s) must be stricly larger than"
                    " `lower` value(s); values were:"
                    "\n upper={0}"
                    "\n lower={1}"
                    "\n offending indices = {2}".format(
                        self.upper, self.lower, list(idx)
                    )
                )
            ...
            zero_from_upper = (
                self.from_lower_upper[1] - self.multiplier**-1 * self.upper
            )
            if not _Mh.vequals_approximately(self.zero, zero_from_upper):
                warnings.warn(
                    "zero computed from upper and lower differ"
                    "\n from upper={0}"
                    "\n from lower={1}"
                    "\n This may be a bug or due to small numerical deviations".format(
                        zero_from_upper, self.zero
                    )
                )
        if zero is not None:
            ...
            if lower is not None:
                warnings.warn(
                    "lower={0} is ignored because zero={1} was given".format(
                        lower, zero
                    )
                )
            if upper is not None:
                warnings.warn(
                    "upper={0} is ignored because zero={1} was given".format(
                        upper, zero
                    )
                )
...
class IntegerMixedFunction2(ComposedFunction):
    ...
    def __init__(
        self, function, integer_variable_indices, operator=np.round, copy=True
    ):
...
class IntegerMixedFunction(ComposedFunction):
    ...
    def __init__(
        self, function, integer_variable_indices, operator=np.floor, copy_arg=True
    ):
        """apply operator(x[i]) for i in integer_variable_indices before to call function(x)"""
        ComposedFunction.__init__(self, [function, self._flatten])
        self.integer_variable_indices = integer_variable_indices
