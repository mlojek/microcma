    ...
    def optimize(
        ...
        **kwargs
        ...
        if kwargs:
            ...
            warnings.warn(
                message
            )  # warnings.simplefilter('ignore', lineno=186) suppresses this warning

        if iterations is not None and min_iterations > iterations:
            warnings.warn(
                "doing min_iterations = %d > %d = iterations"
        ...
        citer, cevals = 0, 0
        with (EvalParallel2 or EvalParallel)(
            objective_fct, None if n_jobs == -1 else n_jobs
        ) as eval_all:
    ...
    def _force_final_logging(self):  # helper function
        ...
        try:
            ...
            try:
                ...
                print(
                    "  The final call of the logger in"
                    " OOOptimizer._force_final_logging from"
                    " OOOptimizer.optimize did not succeed: %s" % str(e)
                )
...
class StatisticalModelSamplerWithZeroMeanBaseClass(object):
    ...
    def parameters(self, mueff=None, lam=None):
        ...
        if (
            hasattr(self, "_mueff")
            and hasattr(self, "_lam")
            and (mueff == self._mueff or mueff is None)
            and (lam == self._lam or lam is None)
        ):
