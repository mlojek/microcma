...
def _clean_up(folder, start_matches, protected):
    ...
    for file_ in os.listdir(folder):
        if any(file_.startswith(s) for s in start_matches) and not any(
            file_.startswith(p) for p in protected
        ):
...
def doctest_files(file_list=files_for_doctest, **kwargs):
    ...
    for file_ in file_list:
        ...
        if verbosity_here >= 0:
            print(
                "doctesting %s ..." % file_,
                " " * (max(len(_file) for _file in file_list) - len(file_)),
                end="",
            )  # does not work in Python 2.5
            sys.stdout.flush()
        protected_files = os.listdir(".")
        report = doctest.testfile(
            file_, package=__package__, **kwargs  # 'cma', # sys.modules[__name__],
        )
...
def main(*args, **kwargs):
    ...
    if len(args) > 0:
        ...
        v = get_version()
        print(
            "doctesting `cma` package%s by calling `doctest_files`:"
            % ((" (v%s)" % v) if v else "")
        )
