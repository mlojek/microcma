...
from .logger import \
    Logger as \
    _Logger  # we can assign _Logger = cma.logger.LoggerDummy to turn off logging
...
from .utilities.utils import is_, rglen
    ...
    def to_dim_times_two(self, bounds):
        ...
        if not bounds:
            ...
            if (
                l[0] != l[1]
                and 1 not in l
                and None not in (bounds[0][-1], bounds[1][-1])
            ):  # disallow different lengths
                raise ValueError(
            ...
            try:
                for i in range(max(l)):
                    b.append(
                        [bounds[0][min((i, l[0] - 1))], bounds[1][min((i, l[1] - 1))]]
                    )
...
class BoundPenalty(BoundaryHandlerBase):
    ...
    **todo**: implement a more generic interface, where this becomes a
    ...
    def update(self, function_values, es):
        ...
        if 11 < 3:  # old
            varis = es.sigma**2 * np.array(
                N * [es.C]
                if np.isscalar(es.C)
                else (  # scalar case
                    es.C
                    if np.isscalar(es.C[0])  # diagonal matrix case
                    else [es.C[i][i] for i in range(N)]
                )
            )  # full matrix case
        else:
    ...
    def update(self, f, g):
        ...
        if self.g is not None and np.any(
            ...
            if 11 < 3 and not self.count and self.chi_domega < 1.05:
                _warnings.warn(
                    "chi_omega=%f as by default, however values <<1.1 may not work well"
                    % self.chi_domega
                )
            ...
            for i in range(len(self.lam)):
                ...
                if (
                    ...
                    or (
                        not is_(self.count_g_in_penalized_domain)
