...
class RecombinationWeights(list):
    ...
    def __init__(self, len_, exponent=1):
        ...
        try:
            ...
            try:  # iterator without len
                ...
                weights = [
                    signed_power(
                        math.log((len_ + 1) / 2.0) - math.log(i), self.exponent
                    )
    ...
    def trim_middle(self, lambda_):
        ...
        if lambda_ < 2:  # somewhat a hack, FIXME
            return int(lambda_) * [0.0]  # maybe this should be 1.?
        mu_ = int(lambda_ / 2)
        assert mu_ > 0
        return self[:mu_] + (lambda_ > 2 * mu_) * [0.0] + self[-mu_:]
    ...
    def finalize_negative_weights(self, dimension, c1, cmu, pos_def=True):
        ...
        if dimension <= 0:
            raise ValueError(
                "dimension must be larger than zero, was " + str(dimension)
            )
    ...
    def do_asserts(self):
        ...
        assert (
            self.mueffminus == 0 == sum(weights[self.mu :])
            or self.mueffminus / 1.001
            < sum(weights[self.mu :]) ** 2 / sum(w**2 for w in weights[self.mu :])
