...
class CMADataLogger(interfaces.BaseDataLogger):
    ...
    def __init__(
        self, name_prefix=default_prefix, modulo=1, append=False, expensive_modulo=1
    ):
        ...
        if os.path.isfile(name_prefix) and name_prefix.endswith(".tar.gz"):
            raise NotImplementedError(
                "sorry, you need to unzip the file manually" " and pass the folder name"
            )
    ...
    def initialize(self, modulo=None):
        ...
        self.last_iteration = (
            0  # some lines are only written if iteration>last_iteration
        )
        if self.modulo <= 0:
            return self
        ...
        try:
            ...
            warnings.warn(
                "could not save `X0` due to this exception:\n    {0}".format(repr(e))
            )
        ...
        try:
            with open(fn, "w") as f:
                f.write(
                    '% # columns="iteration, evaluation, sigma, beta, void, '
        ...
        try:
            with open(fn, "w") as f:
                f.write(
                    '% # columns="iter, evals, sigma, 0, fitness, xbest" '
                    + strseedtime
                    + ", "
                    + self.persistent_communication_dict.as_python_tag
                    + "\n"
    ...
    def load(self, filenameprefix=None):
        ...
        for i in range(len(self.file_names)):
            ...
            try:
                ...
                with warnings.catch_warnings():
                    ...
                    try:
                        ...
                        try:
                            ...
                            ValueError
                        ):  # fit.dat may have different number of columns
                            ...
                            if len(lengths) > 1:
                                print(
                                    "rows in {0} have different lengths {1}, filling in `np.nan`".format(
                                        fn, lengths
                                    )
                ...
                m = utils.format_warning(
                    'reading from file "' + fn + '" failed', "load", "CMADataLogger"
                )
        ...
        for key in self.key_names:
            try:
                ...
                utils.print_warning(
                    "attribute %s missing" % key, "load", "CMADataLogger"
                )
                continue
    ...
    def add(self, es=None, more_data=(), modulo=None):
        ...
        try:
            ...
            iqrangef = np.diff(
                _mathutils.Mh.prctile(es.fit.fit, [25, 75], sorted_=True)
            )[0]
        except:
        ...
        if self.expensive_modulo:
            try:
                ...
                if correlation_matrix is not None and (
                    self.last_precision_matrix is None
                    or eigen_decompositions % self.expensive_modulo == 0
                ):
                    try:
                        self.last_precision_matrix = np.linalg.inv(correlation_matrix)
                        self.last_precision_matrix = _mathutils.to_correlation_matrix(
                            self.last_precision_matrix
                        )
                    except:  # diagonal case
                        warnings.warn(
                            "CMADataLogger failed to compute precision matrix"
                        )
        ...
        try:
            ...
            if self.expensive_modulo:
                for name, matrix in [
                    ["axlencorr.dat", correlation_matrix],
                    ...
                    if (
                        ...
                        if (
                            name not in self.last_correlation_spectrum
                            or eigen_decompositions % self.expensive_modulo == 0
                        ):
                            self.last_correlation_spectrum[name] = sorted(
                                es.opts["CMA_eigenmethod"](matrix)[0] ** 0.5
                            )
                            self._eigen_counter += (
                                matrix is self.last_precision_matrix
                            )  # hack to add only once per for loop
                        c = np.asarray(matrix)
                        ...
                        if 11 < 3:  # old version
                            ...
                            if c_max <= 0:  # no positive values
                                c_max = c_medplus = (
                                    0  # set both "positive" values to zero
                                )
                            elif c_min >= 0:  # no negative values
                                c_min = c_medminus = 0
                        c_min, c_medminus, c_medplus, c_max = _mathutils.Mh.prctile(
                            c, [0, 25, 75, 100]
                        )
                        if (
                            11 < 3
                        ):  # log correlations instead of eigenvalues, messes up KL display
                            _KL = 1e-3 - 0.5 * np.mean(
                                np.log(self.last_correlation_spectrum[name])
                        ...
                        with open(fn, "a") as f:
                            f.write(
                                ...
                                + " "
                                + " ".join(
                                    map(str, self.last_correlation_spectrum[name])
                                )
                                + "\n"
                            )
            ...
            if iteration <= 1:
                utils.print_warning(
                    ("could not open/write file %s: " % fn, sys.exc_info())
                )
    ...
    def select_data(self, iteration_indices):
        ...
        try:
            if len(dat.corrspec):
                dat.corrspec = dat.x[
                    _where([x in iteridx for x in dat.corrspec[:, 0]])[0], :
                ]
        except AttributeError:
            pass
        try:
            if len(dat.precspec):
                dat.precspec = dat.x[
                    _where([x in iteridx for x in dat.precspec[:, 0]])[0], :
                ]
        except AttributeError:
            pass
        return self
    ...
    def plot(
        ...
        try:
            ...
            from matplotlib.pyplot import figure, gcf, subplot
        except ImportError:
            ImportError(
        ...
        if hasattr(self, "es") and self.es is not None:
            ...
            if not self.es.stop() and self.es.countiter > self.last_skipped_iteration:
                ...
                if (
                    self.es.countiter < 3
                    or self.timer_all.elapsed < 0.15
                    or (  # avoid warning when too few data are available
                        self.timer_plot.toc
                        > self.relative_allowed_time_for_plotting * self.timer_all.toc
                    )
                ):
        ...
        figure(fig)
        self.skip_finalize_plotting = (
            True  # disable finalize until end of this plot function
        )
        ...
        if (
            self.fighandle.get_size_inches()[0] / self.fighandle.get_size_inches()[1]
            < 1.4 + 0.5 * addcols
        ):
            self.fighandle.set_figwidth(
                (1 + 0.45 * addcols) * self.fighandle.get_figwidth()
            )
        ...
        if plot_mean:
            self.plot_mean(
                iabscissa,
                x_opt,
                xsemilog=False if plot_mean == "linear" else xsemilog,
                xnormalize=xnormalize,
            )
        else:
            self.plot_xrecent(
                iabscissa, x_opt, xsemilog=xsemilog, xnormalize=xnormalize
            )
        pyplot.xlabel("")
    ...
    def plot_all(
        self, fig=None, iabscissa=0, iteridx=None, foffset=1e-19, x_opt=None, fontsize=7
    ):
        """
    ...
    def plot_axes_scaling(self, iabscissa=0):
        ...
        if np.max(dat.D[:, 5:]) == np.min(dat.D[:, 5:]):
            pyplot.text(
                0,
                dat.D[-1, 5],
                "all axes scaling values equal to %s" % str(dat.D[-1, 5]),
                verticalalignment="center",
            )
            return self  # nothing interesting to plot
        self._enter_plotting()
        color = iter(
            pyplot.get_cmap("plasma_r")(np.linspace(0.35, 1, dat.D.shape[1] - 5))
        )
        _x = _monotone_abscissa(dat.D[:, iabscissa], iabscissa)
        for i in range(5, dat.D.shape[1]):
            pyplot.semilogy(_x, dat.D[:, i], "-", color=next(color))
        # pyplot.hold(True)
        smartlogygrid()
    ...
    def plot_stds(self, iabscissa=0, idx=None):
        ...
        if 1 < 2 and dat.std.shape[1] < 100:
            ...
            for i, s in enumerate(annotations):
                ...
                pyplot.text(_x[-1], dat.std[-1, 5 + i], " " + str(s))
        else:
            pyplot.semilogy(_x, dat.std[:, 5:], "-")
        # pyplot.hold(True)
        smartlogygrid()
    ...
    def plot_sigvec(self, iabscissa=0, idx=None):
        ...
        if 1 < 2 and dat.shape[1] < 100:
            ...
            # idx2 = np.argsort(idx)
            dat[-1, 5 + idx] = np.logspace(
                np.log10(np.min(dat[:, 5:])),
                np.log10(np.max(dat[:, 5:])),
                dat.shape[1] - 5,
            )
    ...
    def plot_mean(
        self, iabscissa=0, x_opt=None, annotations=None, xsemilog=None, xnormalize=None
    ):
    ...
    def plot_correlations(self, iabscissa=0, name="corrspec"):
        ...
        from matplotlib.pyplot import axis, gca, semilogy, text, title
        ...
        # semilogy(x, y, '-c')
        color = iter(pyplot.get_cmap("plasma_r")(np.linspace(0.35, 1, y.shape[1])))
        ...
        title(
            "Spectrum (roots) of %s matrix"
            % ("white precision" if name.startswith("prec") else "correlation")
        )
        # pyplot.xticks(xticklocs)
        self._xlabel(iabscissa)
        self._finalize_plotting()
    ...
    def plot_divers(self, iabscissa=0, foffset=1e-19, fshift=0, message=None):
        ...
        """
        from matplotlib import pyplot
        from matplotlib.pyplot import axis, semilogy, text, title
        ...
        if dat.f.shape[1] > 8:  # interquartile f-range
            # semilogy(dat.f[:, iabscissa], abs(dat.f[:,[6, 7, 10, 12]])+foffset,'-k')
            semilogy(
                _x, abs(dat.f[:, [8]]) + foffset, "grey", linewidth=0.7
            )  # darkorange is nice
        ...
        idx = _where(sgn < 0)[0]
        semilogy(_x[idx], abs(dat.f[idx, 5]) + foffset, ".m")  # , markersize=5
        ...
        for istart in start_idx:
            ...
            if istart > 0:  # line to the left of istart
                semilogy(
                    _x[istart - 1 : istart + 1],
                    abs(dat.f[istart - 1 : istart + 1, 5]) + foffset,
                    "--m",
                )
            if istop:  # line to the left of istop
                semilogy(
                    _x[istop - 1 : istop + 1],
                    abs(dat.f[istop - 1 : istop + 1, 5]) + foffset,
                    "--m",
                )
                # mark the respective first positive values
                semilogy(_x[istop], abs(dat.f[istop, 5]) + foffset, ".b", markersize=7)
            # mark the respective first negative values
            semilogy(_x[istart], abs(dat.f[istart, 5]) + foffset, ".r", markersize=7)
        ...
        text(_x_std[-2], max(dat.std[-2, 5:]), "max std", fontsize=fontsize)
        text(_x_std[-2], min(dat.std[-2, 5:]), "min std", fontsize=fontsize)
        ...
        for dfit, label in [
            ...
            if any(idx):
                ...
                if not len(idx_nan):  # should never happen
                    ...
                    for i_end in idx_nan:
                        if i_end > i_start:
                            semilogy(_x[i_start:i_end], dfit[i_start:i_end], "-c")
                        i_start = i_end + 1
                    if len(dfit) > idx_nan[-1] + 1:
                        semilogy(_x[idx_nan[-1] + 1 :], dfit[idx_nan[-1] + 1 :], "-c")
                text(_x[-1], dfit[idx][-1], label, fontsize=fontsize + 2)
            ...
            if 11 < 3:  # delta-fitness as points
                ...
                semilogy(_x[1:], np.abs(dfit) + foffset, ".c")  # abs(fit(g) - fit(g-1))
                i = dfit > 0
                # print(np.sum(i) / float(len(dat.f[1:,iabscissa])))
                semilogy(
                    _x[1:][i], np.abs(dfit[i]) + foffset, ".r"  # abs(fit(g) - fit(g-1))
                )
            # postcondition: dfit, idx = dfit1, ...

        # fat red dot for overall minimum
        i = np.argmin(dat.f[:, 5])
        semilogy(_x[i], np.abs(dat.f[i, 5]), "ro", markersize=9)
        if any(idx):  # another fat red dot
            semilogy(_x[i], dfit[idx][np.argmin(dfit[idx])] + 1e-98, "ro", markersize=9)
        # semilogy(dat.f[-1, iabscissa]*np.ones(2), dat.f[-1,4]*np.ones(2), 'rd')

        # AR and sigma
        semilogy(_x, dat.f[:, 3], "-r")  # AR
        semilogy(_x, dat.f[:, 2], "-g")  # sigma
        text(_x[-1], dat.f[-1, 3], r"axis ratio", fontsize=fontsize)
        text(_x[-1], dat.f[-1, 2] / 1.5, r"$\sigma$", fontsize=fontsize + 3)
        ax = array(axis())
        # ax[1] = max(minxend, ax[1])
        axis(ax)
        text(
            ax[0] + 0.003 * (ax[1] - ax[0]),
            ax[2] * (ax[3] / ax[2]) ** 0.002,
        ...
        if np.size(dat.sigvec) > 1:  # try to figure out whether we have data
            ...
            semilogy(
                _x, 1 / dat.sigvec[:, 3], "k", label="$\\beta=\\sqrt{cond(CORR)} - 1$"
            )
            text(_x[-1], 1 / dat.sigvec[-1, 3], "dd-damp$\\approx1/\\sqrt{cond(CORR)}$")
            semilogy(
                _x,
                np.max(dat.sigvec[:, 5:], axis=1) / np.min(dat.sigvec[:, 5:], axis=1),
                "darkred",
                label="axis ratio of diagonal decoding",
            )
            text(
                _x[-1], np.max(dat.sigvec[-1, 5:]) / np.min(dat.sigvec[-1, 5:]), "dd-AR"
            )
        if np.size(dat.corrspec) > 1:
            ...
            semilogy(
                _x, c_odds(dat.corrspec[:, 2]), "c", label=r"$\min (c + 1) / (c - 1)$"
            )
            semilogy(
                _x, c_odds(dat.corrspec[:, 5]), "c", label=r"$\max (c + 1) / (c - 1)$"
            )
            text(
                _x[-1],
                c_odds(np.asarray([dat.corrspec[-1, 2]])),
                r"$\max (c + 1) / (c - 1)$",
            )
            text(
                _x[-1],
                c_odds(np.asarray([dat.corrspec[-1, 5]])),
                r"$-{\min}^{-1} (c + 1)\dots$",
            )
    ...
    def _finalize_plotting(self):
        ...
        pyplot.rcParams["font.size"] = (
            self.original_fontsize
        )  # changes font size in current figure which defeats the original purpose
    ...
    def _plot_x(
        ...
        if not hasattr(self, "x"):
            utils.print_warning(
                "no x-attributed found, use methods " + "plot_xrecent or plot_mean",
                "plot_x",
                "CMADataLogger",
            )
            return
        if annotations is None:
            annotations = self.persistent_communication_dict.get("variable_annotations")
        import matplotlib
        from matplotlib.pyplot import axis, grid, plot, text, title, yscale
        ...
        if np.isscalar(x_opt):  # interpret as iteration number or negative index
            if x_opt == 0:  # subtract X0
                ...
                try:
                    ...
                    warnings.warn(
                        "could not open file {0} to subtract ``X0``".format(fn)
                    )
                except Exception as e:
                    warnings.warn(
                        "could not read `X0` due to this exception:\n    {0}".format(
                            repr(e)
                        )
                    )
                ...
                if self.x[_i, 0] != x_opt:
                    warnings.warn(
                        ...
                        " of iteration {3} from `x_opt` argument".format(
                            self.x[_i, 0], _i, _m, x_opt
                        )
                    )
                x_opt = self.x[_i, 5:]
            dat_x = dat.x[:, :]
            ...
            x_opt is not None
        ):  # x_opt is a vector or an index array to reduce dimension or both
            dat_x = dat.x[:, :]
            try:
                dat_x[:, 5:] -= x_opt
        ...
        if dat_x.shape[1] < 100:
            ...
            if xsemilog or (xsemilog is None and remark and remark.startswith("mean")):
                ...
                # idx2 = np.argsort(idx)
                dat_x[-1, 5 + idx] = np.linspace(
                    np.min(dat_x[:, 5:]), np.max(dat_x[:, 5:]), dat_x.shape[1] - 5
                )
        else:
            minxend = 0
        self._enter_plotting()
        ...
        if xsemilog or (xsemilog is None and remark and remark.startswith("mean")):
            ...
            if len(_d_pos):
                if matplotlib.__version__[:3] < "3.3":
                    ...
                    yscale(
                        "symlog", linthreshy=np.min(_d_pos)
        ...
        if dat_x.shape[1] < 100:  # annotations
            ...
            plot(np.dot(_x[-2], [1, 1]), array([ax[2] + 1e-6, ax[3] - 1e-6]), "k-")
            ...
            for i in range(len(idx)):
                ...
                text(
                    ...
                    + utils.num2str(
                        dat_x[-2, 5 + i], significant_digits=2, desired_length=4
                    ),
                )
        smartlogygrid()
        ...
        while i < len(dat.f) and dat.f[-i][0] == dat.f[-1][0]:
            i += 1
        popsi = (
            (dat.f[-1][1] - dat.f[-i][1]) / (dat.f[-1][0] - dat.f[-i][0])
            if len(dat.f.T[0]) > 1 and dat.f[-1][0] > dat.f[-i][0]
            else None
        )
        mpopsi = (
            (dat.f[-1][1] - dat.f[0][1]) / (dat.f[-1][0] - dat.f[0][0])
            if dat.f[-1][0] > dat.f[0][0]
            else None
        )
        title(
            "Object Variables ("
    ...
    def downsampling(self, factor=10, first=3, switch=True, verbose=False):
        ...
        for name in self.file_names:
            if not os.path.isfile(self.name_prefix + name + ".dat"):
                warnings.warn(
                    "CMADataLogger.downsampling: file {0} not found".format(
                        self.name_prefix + name + ".dat"
                    )
                )
                continue
            ...
            if verbose and iline > first:
                print(
                    "%d" % (cwritten) + " lines written in " + newprefix + name + ".dat"
                )
        if switch:
...
def plot(
    ...
    **kwargs
    ...
    return CMADataLogger(name).plot(
        fig,
        abscissa,
        iteridx,
        plot_mean,
...
def plot_zip(name=None, filename=None, unique=True):
    ...
    """
    return CMADataLogger(name if name else CMADataLogger.default_prefix).zip(
        filename=filename, unique=unique
    )
...
class Logger(object):
    ...
    def __init__(
        ...
        if obj_or_name == str(obj_or_name) and attributes is not None:
            raise ValueError(
                "string obj %s has no attributes %s"
    ...
    def _create_path(self, name_prefix=None):
        ...
        path = os.path.abspath(
            os.path.join(*[a for a in os.path.split(name_prefix) if a])
        )
    ...
    def _autoname(self, obj):
        ...
        if not os.path.isfile(
            self._name
        ):  # we want to be able to load an existing logger
    ...
    def _unique_name_addition(self, name=None):
        ...
        if i % 99 == 0:
            utils.print_message(
                "%d Logger data files like %s found. \n"
    ...
    def plot(self, plot=None, clear=True, transformations=None):
        ...
        if m < 2:  # data cannot be indexed like data[:,0]
            ...
            color = iter(
                plt.get_cmap("plasma")(np.linspace(0.01, 0.9, m))
            )  # plasma was: winter_r
            idx_labels = [
                int(i * m / len(self.labels)) for i in range(len(self.labels))
            ]
            if len(idx_labels) > 1:
    ...
    if scale.startswith("symlog"):
        try:
            ...
            if s < 1e3:
                plt.grid(
                    True, which="minor"
                )  # minor grid isn't yet supported in symlog
        return
...
def custom_default_matplotlib():
    ...
    plt.margins(x=0.01, y=0.01)  # mpl.rcParams['axes.xmargin'] = 0.01, default is 0.05
