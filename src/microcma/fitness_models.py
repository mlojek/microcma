    ...
    def __call__(self, X):
        ...
        if 11 < 3:
            ...
            if model.size < self.settings.minimum_model_size:
                evals.eval_sequence(
                    self.settings.minimum_model_size - model.size,
                    self.fitness,
                    model.add_data_row,
                )
                self.evaluations += evals.evaluations
                model.sort(self.settings.model_sort_globally or evals.evaluations)
                return evals.surrogate_values(
                    model.eval, self.settings.return_true_fitnesses
                )
...
class LQModel(object):
    ...
    def add_data_row(self, x, f, prune=True, force=False):
        ...
        if 11 < 3:
            ...
            self.X = np.vstack(
                [x] + ([self.X] if self.count > 0 else [])
            )  # stack x on top
            self.Z = np.vstack(
                [self.expand_x(x)] + ([self.Z] if self.count > 0 else [])
            )
